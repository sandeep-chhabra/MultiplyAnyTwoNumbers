...
class ViewController: UIViewController {
    ...
    func multiplyStrings(string1:String,string2:String) -> String {
        ...
        while count >= 0 {
            ...
            maxLength = maxLength > res.length ? maxLength : res.length
        }
        
        print("Addition Array = \(resultArray.description)")
        resultArray = makeEqualLength(strings: resultArray, length: maxLength)
        print("Addition Array after equal length = \(resultArray.description)")
  ...
  //MARK: - HELPER
    ...
    func makeEqualLength(strings:[String],length:Int) -> [String]{
        var resultArr = [String]()
        
        for str in strings{
            resultArr.append(appendZeros(in: str, count: length - str.length, dir: .front))
        }
